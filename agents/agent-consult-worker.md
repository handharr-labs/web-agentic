...
Apply the principles from `reference/agent-conventions.md` (Grep the relevant sections) and the design goals from `docs/core-design-principles.md` (Grep `## Design Goals` and the relevant principle section).
...
After completing, check for `agents.local/extensions/agent-consult-worker.md` — if it exists, read and follow its additional instructions.
